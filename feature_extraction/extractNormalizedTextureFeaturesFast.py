...
def postProcessNetworkOutput(pred, class_labels, classes): # no GT needed
    ...
    # Remove disconnected blobs that don't belong to dominant label TODO check
    pred_binary = pred > 0
    labeled_array, num_features = label(pred_binary)

    if num_features > 1: #TODO check
    ...
    if len(labels) > 1:
        ...
        unique_labels, counts = np.unique(pred, return_counts=True)
        counts_fg = counts[unique_labels >0] # TODO figure out why this code
        dominant_label = labels[np.argmax(counts_fg)]
...
def process_file(file, fold, pred_fold, img_fold, muscle, classes, class_labels):
    ...
    pred_out, class_pred = postProcessNetworkOutput(pred, class_labels, classes)
